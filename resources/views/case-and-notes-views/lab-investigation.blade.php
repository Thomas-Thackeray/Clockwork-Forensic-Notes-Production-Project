            ...
            <section class = 'white-box full-width'>
                ...
                <section class = 'large-grouping-form flex-column'>
                    ...
                    <span class = 'form-column-grouping flex-column'>
                        <input value = "{{ old('noteTitle_input') }}" class = 'MR-20 MB-10 thirty-percent-width' type="text" name="noteTitle_input" placeholder = "Note Title">
                        @error('noteTitle_input')<p class="validation_message warning">{{ $message }}</p>@enderror
                    </span>
                    ...
                    <span class = 'form-row-grouping flex-row'>
                        <select value = "{{ old('interactedEvidence_input') }}" name = 'interactedEvidence_input' class = 'MR-20 thirty-percent-width'>
                            <option value = ''>--- Please Select The Evidence Interacted With ---</option>
                        ...
                        @error('interactedEvidence_input')<p class="validation_message warning">{{ $message }}</p>@enderror
                        
                        <select value = "{{ old('actionType_input') }}" name = 'actionType_input' class = 'MR-20 thirty-percent-width'>
                            <option value = ''>--- Action Type ---</option>
                            <option value = 'tool used'>Tool Used</option>
                            <option value = 'forensic imaging process'>Forensic Imaging Process</option>
                            <option value = 'file exploration'>File Exploration</option>
                            <option value = 'evidence handover'>Evidence Hand-Over</option>
                            <option value = 'other'>Other</option>
                        </select>
                        @error('actionType_input')<p class="validation_message warning">{{ $message }}</p>@enderror

                        <select value = "{{ old('outcome_input') }}" name = 'outcome_input' class = 'MR-20 thirty-percent-width'>
                            <option value = ''>--- Outcome ---</option>
                            <option value = 'success'>Success</option>
                            <option value = 'failure'>Failure</option>
                            <option value = 'evidence found'>Evidence Found</option>
                            <option value = 'metadata found'>Metadate Uncovered</option>
                            <option value = 'deleted files retrived'>Deleted File(s) Retrived</option>
                            <option value = 'personal details found'>Name(s), Address(s), Contact Detail(s) Found</option>
                            <option value = 'other'>Other</option>
                        </select>
                        @error('outcome_input')<p class="validation_message warning">{{ $message }}</p>@enderror
                    </span>

                    <span class = 'form-column-grouping flex-column'>
                        <textarea name = 'actionDescription_input' class = 'MB-10' placeholder = "Description Of Actions">{{ old('actionDescription_input') }}</textarea>
                        <textarea name = 'actionResult_input' class = 'MB-10' placeholder = "Result Of Actions">{{ old('actionResult_input') }}</textarea>
                    </span>

                    <span class = 'form-row-grouping flex-row'>
                        <input name = 'toolUsedName_input' class = 'MR-20 thirty-percent-width' type="text" placeholder = "Name Of Tool" value = "{{ old('toolUsedName_input') }}">
                        @error('toolUsedName_input')<p class="validation_message warning">{{ $message }}</p>@enderror
                        <input name = 'toolUsedVersion_input' class = 'MR-20 thirty-percent-width' type="text" placeholder = "Version" value = "{{ old('toolUsedVersion_input') }}">
                        @error('toolUsedVersion_input')<p class="validation_message warning">{{ $message }}</p>@enderror
                    </span>                    
