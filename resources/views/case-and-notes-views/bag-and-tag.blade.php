<x-app-layout :case_name="$case_name" :caseID="$caseID">
    ...
    <section id = 'mainContent-container' class = 'flex-row flex-wrap'>
        ...
        <form action = '/case/{{$case_name}}/submit-note/bag-and-tag' method = 'POST' enctype='multipart/form-data' class = 'flex-row flex-wrap full-width'>
        ...
        <section id = 'note-body'>
            ...
            <section class = 'white-box full-width'>
                ...
                <section class = 'large-grouping-form flex-column'>
                    ...
                    <h3>Evidence Deatils</h3>

                    <?php  
                    $caseAssigned = auth()->user()->companyConnect->id; 
                    $EvidenceNames  = DB::table('fornsic_notes')->where('note_type','=', 'Evidence Identification')->where('case_assigned', '=', $caseID)->get();                       
                    $result3 = json_decode($EvidenceNames, true);
                    $device1 = $result3[0]["device_1_Name"]; $device2 = $result3[0]["device_2_Name"]; $device3 = $result3[0]["device_3_Name"]; $device4 = $result3[0]["device_4_Name"];
                    $device5 = $result3[0]["device_5_Name"]; $device6 = $result3[0]["device_6_Name"]; $device7 = $result3[0]["device_7_Name"]; $device8 = $result3[0]["device_8_Name"];
                    $device9 = $result3[0]["device_9_Name"]; $device10 = $result3[0]["device_10_Name"];
                    ?>                       

                    

                    <span class = 'form-row-grouping flex-row'>
                        <select name = 'evidenceName_input' value = "{{ old('evidenceName_input') }}" class = 'MR-20 thirty-percent-width'>
                            <option value = ''>--- Evidence Name / Ref ---</option>
                            <?php
                            if($device1 != 'N/S'){
                                ?>
                                    <option value = '<?php echo $device1 ?>'><?php echo $device1 ?></option>
                                <?php
                            }  
                            if($device2 != 'N/S'){
                                ?>
                                    <option value = '<?php echo $device2 ?>'><?php echo $device2 ?></option>
                                <?php
                            }
                            if($device3 != 'N/S'){
                                ?>
                                    <option value = '<?php echo $device3 ?>'><?php echo $device3 ?></option>
                                <?php
                            }    
                            if($device4 != 'N/S'){
                                ?>
                                    <option value = '<?php echo $device4 ?>'><?php echo $device4 ?></option>
                                <?php
                            }    
                            if($device5 != 'N/S'){
                                ?>
                                    <option value = '<?php echo $device5 ?>'><?php echo $device5 ?></option>
                                <?php
                            }              
                            if($device6 != 'N/S'){
                                ?>
                                    <option value = '<?php echo $device6 ?>'><?php echo $device6 ?></option>
                                <?php
                            }    
                            if($device7 != 'N/S'){
                                ?>
                                    <option value = '<?php echo $device7 ?>'><?php echo $device7 ?></option>
                                <?php
                            }    
                            if($device8 != 'N/S'){
                                ?>
                                    <option value = '<?php echo $device8 ?>'><?php echo $device8 ?></option>
                                <?php
                            }    
                            if($device9 != 'N/S'){
                                ?>
                                    <option value = '<?php echo $device9 ?>'><?php echo $device9 ?></option>
                                <?php
                            }    
                            if($device10 != 'N/S'){
                                ?>
                                    <option value = '<?php echo $device10 ?>'><?php echo $device10 ?></option>
                                <?php
                            }    
                    ...
                    <span class = 'form-column-grouping flex-column'>
                        <input class = 'MR-20 MT-10 thirty-percent-width' type="text" name="evidenceReferenceNumber_input" placeholder = "Evidence Reference Number" value = "{{ old('evidenceReferenceNumber_input') }}">
                        @error('evidenceReferenceNumber_input')<p class="validation_message warning">{{ $message }}</p>@enderror
                    </span>

                    <span class = 'form-column-grouping flex-column'>
                        <textarea name = 'evidenceDescription_input'  placeholder = "Evidence Description">{{ old('evidenceDescription_input') }}</textarea>
                        @error('evidenceDescription_input')<p class="validation_message warning">{{ $message }}</p>@enderror
                    </span>

                    <span class = 'form-column-grouping flex-row'>
                        <input class = 'MR-20 thirty-percent-width' type="text" name="manufacturer_input" value = "{{ old('manufacturer_input') }}" placeholder = "Manufacturer">
                        <input class = 'MR-20 thirty-percent-width' type="text" name="model_input" value = "{{ old('model_input') }}" placeholder = "Model">
                        <input class = 'MR-20 thirty-percent-width' type="text" name="serialNumber_input" value = "{{ old('serialNumber_input') }}" placeholder = "Serial Number">
                    </span>

                    <h3>Evidence Bag Deatils</h3>

                    <span class = 'form-column-grouping flex-column'>
                        <input class = 'MR-20 MT-10 thirty-percent-width' type="text" name="uniqueId_input" value = "{{ old('uniqueId_input') }}" placeholder = "Unique ID">
                        @error('uniqueId_input')<p class="validation_message warning">{{ $message }}</p>@enderror
                        <input class = 'MR-20 MT-10 thirty-percent-width' type="text" name="courtExhibitNumber_input" value = "{{ old('courtExhibitNumber_input') }}" placeholder = "Court Exhibit Number">
                        @error('courtExhibitNumber_input')<p class="validation_message warning">{{ $message }}</p>@enderror
                        <input class = 'MR-20 MT-10 thirty-percent-width' type="text" name="storageReference_input" value = "{{ old('storageReference_input') }}" placeholder = "Storage Reference">
        ...
        <input id = 'start_time' type = 'hidden' name = 'start_date' value = '<?php echo date('Y-m-d'); ?>'>
        <input id = 'start_date' type = 'hidden' name = 'start_time' value = '<?php echo date('h:i:s'); ?>'>
